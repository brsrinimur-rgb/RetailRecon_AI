...
def read_upload(upload):
    ...
    if name.endswith(".csv"):
        ...
        hr=_best_header_row(raw)
        return {"CSV":pd.read_csv(io.BytesIO(data),header=hr)}

    xls=pd.ExcelFile(io.BytesIO(data))
    result={}
    for sheet in xls.sheet_names:
        raw=pd.read_excel(xls,sheet_name=sheet,header=None,nrows=20)
        hr=_best_header_row(raw)
        result[sheet]=pd.read_excel(xls,sheet_name=sheet,header=hr)
    return result
