...
def _norm_payment(v):
    ...
    # V72: GCC NET was already a recognized payment type in core.py (commission
    # rates, JV grouping) but was never normalized on the bank-narration side,
    # so GC_-prefixed ANB credits never got tagged with a Provider/Scheme and
    # could never be considered for matching at all.
    if s in {"GC","GCC","GCCNET"}: return "GCC NET"
...
def parse_anb_narration(parts):
    ...
    # V72: added GC (GCC NET) alongside MADA/VC/MC. Real data confirmed a
    # POS GC_... narration for GCC NET credits that this regex did not
    # previously match at all -- these credits carried no Provider/Scheme
    # tag and could never be considered for matching (V70 Finding 3,
    # SAR 5,354.98 / 12 rows). Assumed to share the same
    # <SCHEME>_<VAT>_<FEE>_TX_<COUNT> shape as every other ANB card scheme,
    # matching how core.py already treats GCC NET as a card-style scheme
    # (its own commission rate, its own JV grouping) rather than a BNPL
    # provider -- to be confirmed against the real narration text on rerun.
    m=re.search(r"\b(MADA|VC|MC|GC)_([0-9.]+)_([0-9.]+)_TX_(\d+)\b",raw,re.I)
    if m:
    ...
    provider="ANB POS" if scheme in {"MADA","VISA","MASTERCARD","GCC NET"} else ""
