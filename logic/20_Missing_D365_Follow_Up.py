...
def _load_missing_d365_from_report(uploaded) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "Transaction Date": exc["POS Date"] if "POS Date" in exc.columns else "",
        "Transaction Time": (
...
st.caption(
    "The current reconciliation report does not contain Card Number or Transaction Time. "
    "Upload the original POS/provider files here to enrich those two display fields. "
    "This is follow-up data only; it is not used for matching."
)
