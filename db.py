...
def ensure_store_mapping_master_schema(conn=None):
    """
    Self-healing migration for store_mapping_master, mirroring
    ensure_correction_log_schema() above exactly. Called at query time
    (immediately before both the SELECT in load_store_mapping_master() and
    the INSERT/UPDATE in save_store_mapping_master()), not just relied on
    at module-import time via init_db()/migrate_database().

    This closes a real production gap: init_db() DOES call
    migrate_database(conn) at import time, and migrate_database() DOES
    call _ensure_columns() for this table -- so in principle the
    d365_store_display_name column should already exist by the time any
    page runs. But correction_log had the exact same theoretical coverage
    before V20, and still needed this same query-time safety net added
    (see V20's "Self-Healing Database Schema" note: "migration performed
    ... immediately before load_correction_log SELECT" was added because
    import-time migration alone still left a real production gap). Same
    fix, same reasoning, applied here.
    ...
    _ensure_columns(conn,"store_mapping_master",[
        ("store_code","TEXT"),("active","TEXT"),("notes","TEXT"),
        ("updated_at","TEXT"),("d365_store_display_name","TEXT"),
    ])
...
def load_store_mapping_master():
    conn=get_conn()
    ensure_store_mapping_master_schema(conn)
    df=pd.read_sql_query(
...
def save_store_mapping_master(df,mode="merge",user=None):
    if df is None:
        return
    ensure_store_mapping_master_schema()
    cols={str(c).strip().lower():c for c in df.columns}
