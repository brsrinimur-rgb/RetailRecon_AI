...
def _find_payment(q):
    ...
    #
    # BUG FIX (2026-09-11): that check alone isn't enough. difflib's ratio()
    # can score an ordinary English word as "close enough" (>=0.72) to a
    # SHORT payment alias purely by chance -- found via real testing:
    # "what came in through the tills" scored "came" at 0.75 against "amex"
    # (their shared middle letters "ame" line up), which silently filtered
    # an unrelated question down to AMEX-only. "cast"/"wash"/"dash" score
    # similarly against "cash". A pure similarity-ratio cutoff cannot tell
    # these apart from genuine typos of the SAME length and ratio (e.g.
    # "vise" vs "visa" also scores exactly 0.75, and that fuzzy-correction
    # is real, existing, intentional behavior this fix must not break).
    #
    # Fix: also require the token to share the payment alias's FIRST
    # LETTER. A real typo of a word overwhelmingly preserves its first
    # letter ("vise"/visa, "mastercart"/mastercard, "amx"/amex, "tammara"/
    # tamara, "caash"/cash all keep letter 1) -- an unrelated English word
    # that merely happens to share some interior letters with a payment
    # name generally does not ("came" vs amex: c vs a; "wash"/"dash" vs
    # cash: w/d vs c). This keeps every documented typo-tolerance case
    # working (verified by REGRESSION_AI_COPILOT_ADVANCED's "mastercart"/
    # "vise" assertions) while closing the real false-positive found above.
    for tok in re.findall(r"[a-z]{3,}", ql):
        same_first_letter=[c for c in _PAYMENT_FUZZY_CANDIDATES if c[:1]==tok[:1]]
        match=difflib.get_close_matches(tok, same_first_letter, n=1, cutoff=0.72)
        if match:
